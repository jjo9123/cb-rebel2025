...
<script>
   document.addEventListener("DOMContentLoaded", function () {
            ...
            box.addEventListener("mouseenter", function () {
                ...
                if (newContent) {
                    ...
                    if (lastActiveContent && lastActiveContent !== newContent) {
                        ...
                        lastActiveContent.style.maxHeight = "0px"; // Collapse height
                    ...
                    requestAnimationFrame(() => {
                        const contentHeight = newContent.scrollHeight + 20; // Ensure padding is included
                        newContent.style.maxHeight = `${contentHeight}px`;
                        
                        // Delay adding padding slightly to avoid cut-off
                        setTimeout(() => {
                            newContent.style.padding = "20px"; 
                        }, 50); // Small delay ensures height is calculated first
    ...
    function applyBehavior() {
        document.querySelectorAll(".boxes .acf-boxes-container").forEach(block => {
            if (!isMobile()) {
                enableHover(block);
            }
        });
